    ...
    >
      {events.map((event) => (
        <Box
          key={event.name}
          textAlign="center"
          width={isSmallerScreen ? "80px" : "80px"}
          _hover={{ color: "rgb(37 99 235)" }}
        >
          <Button
            size={isSmallerScreen ? "lg" : "xl"}
            borderRadius="50%"
            variant="outline"
            justifyContent="center"
            padding={isSmallerScreen ? 15 : 6}
            _hover={{
              bg: "rgb(239 246 255)",
              color: "rgb(37 99 235)",
          ...
          >
            <Center>
              <event.icon size={isSmallerScreen ? 25 : 20} />
            </Center>
          </Button>
          <Text mt={2} fontSize={isSmallerScreen ? "sm" : "md"}>
            {event.name}
          </Text>
        </Box>
      ))}
